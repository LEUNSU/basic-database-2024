...
userpass = 'mssql_p@ss'
dbName = 'Madang'
dbcharset = 'UTF8' # EUC-KR로 사용하면 안 됨
...
class qtApp(QMainWindow):
    ...
    def btnSaveClicked(self):   # 저장버튼 클릭
        ...
        if isValid == False: # 위 입력값 중에 하나라도 빈 값이 존재한다면
            QMessageBox.warning(self, '저장경고', waringMsg)
            return       

        ## mode = 'I'일 때는 중복번호를 체크해야 하지만, mode = 'U'일 때는 체크해서 막으면 수정 자체가 안 됨
        print(mode)
        if mode == 'I': # INSERT인 경우
            # 2. 현재 존재하는 번호를 사용했는지 체크, 이미 있는 번호면 DB입력 쿼리 실행이 안 되도록 막아야 함
            conn = pymssql.connect(server=serverName, user=userId, password=userpass, database=dbName, charset=dbcharset) 
            cursor = conn.cursor(as_dict=False) # COUNT(*)는 데이터가 딱 1개이기 때문에 as_dict=False로 해야 함
            query = f'''SELECT COUNT(*)
                        FROM Book
                        WHERE bookid = {self.txtBookId.text()}''' # 현재 입력하고자하는 번호가 있는 확인쿼리
            cursor.execute(query)
            # print(cursor.fetchone()[0]) # COUNT(*)는 데이터가 딱 1개이기 때문에 cursor.fetchone() 함수로 (1, ) 튜플을 가져 옴
    ...
    def tblBooksSelected(self): # 조회결과 테이블위젯 내용 클릭
        ...
        # 모드를 UPDATE로 변경
        global mode # 전역변수를 내부에서 사용함
        mode = 'U'
        # txtBookId를 사용하지 못 하게 설정
