-- 서브쿼리 리뷰
-- WHERE절
-- All, ANY(SOME) -> ANY(SOME)을 쓰는 조건은 아래의 데이터에서 사용이 어려움
...
-- 전체 판매액 118000, 대한민국 고객 총 판매액 46000
...
-- SELECT절 서브쿼리, JOIN으로 변경가능 이미 쿼리가 너무 복잡해서 더 이상 테이블을 추가하기 힘들면,
-- 많이 사용
-- 51. 고객별 판매액 보이시오, GROUP BY가 들어가면 SELECT절에 반드시 집계함수가 들어가야 함!!!
...
-- 52. 고객별 판매액을 보이시오(서브쿼리 --> 조인)
-- 고객별 판매액 집계 쿼리가 FROM절에 들어가면 무조건 모든 속성(컬럼)에 이름이 지정되어야 함
SELECT b.total
     , c.name
